	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussliuyanban`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussliuyanban`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussliuyanban`, this.ruleForm);
						}else{
							await this.$api.add(`discussliuyanban`, this.ruleForm);
						}
						uni.setStorageSync('discussliuyanbanCleanType',true);
